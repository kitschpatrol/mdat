...
import MagicmarkError from './magicmark-error'
...
export type ExpandFileReport = {
	expandedFile: string
	report: MagicmarkError[]
	sourceFile: string
}
export type ExpandStringReport = { expandedString: string; report: MagicmarkError[] }
export type ExpandAstReport = { expandedAst: Root; report: MagicmarkError[] }
...
export async function expandAst(ast: Root, options: ExpandAstOptions): Promise<ExpandAstReport> {
	...
	for (const newContentItem of newContent) {
		...
		try {
			...
			if (error instanceof MagicmarkError) {
				report.push(error)
	...
	if (meta && metaNode === undefined) {
		report.push(new MagicmarkError(`Missing meta comment`, 'error', 'meta'))
	...
	for (const [keyword, rule] of Object.entries(resolvedRules)) {
		if (rule.required && !newContent.some(({ keyword: usedKeyword }) => usedKeyword === keyword)) {
			report.push(
				new MagicmarkError(`Missing required rule "${keyword}"`, 'error', 'required', undefined, {
					keyword: rule,
				}),
			)
		}
	...
	try {
		...
		if (error instanceof Error) {
			report.push(new MagicmarkError(`Markdown AST invalid`, 'error', 'markdown'))
		}
	}
